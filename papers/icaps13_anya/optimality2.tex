...
we show (i) the optimal path 
appears in the search space 
and (ii) when the target is expanded we 
have found an optimal path.
%its $f$-value
%is the length of the optimal path.
%the $f$-value of the goal node, when 
%expanded, is minimal
%is expanded with a
%a minimal $f$-function defined in the previous 
%section is both admissible and monotonic.
%two conditions that 
%are both neccessary and sufficient conditions to guarantee optimality.
%conditions which are sufficient to guarantee optimality.
%guaranteeing optimality.
%and thus applicable to optimal search. 
...
\begin{proof}
  ...
  mentioned in Figure~\ref{fig::ex1}) or a corner point on the same row.  
...
We now assume the search space is explored 
by A* search employing Equation~(\ref{eq::f}).
%by always expanding the node with minimal $f$-value
%according to Eq.~(\ref{eq::f}).  
