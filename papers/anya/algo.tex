...
\end{proof}
From Lemma~\ref{lemma::taut_successors} we conclude that Anya is
complete: for any given search node $(I, r)$ every point on a taut local path
from $r$ through $I$ is reachable via some successor $(I', r')$. As we only have positive
cost transitions, from a node to each successor, we can be certain that if a 
path exists it will eventually be returned. We now show that any path 
returned by Anya is also cost optimal.
\begin{theorem}
