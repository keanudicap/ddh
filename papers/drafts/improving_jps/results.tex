...
\begin{figure}[tb] \begin{center}
       ...
       \end{center}
		\vspace{-1em}
	   \caption{We measure the relative performance (or improvement factor) of each of our new JPS variants 
...
factor by several times but the end result in terms of time-per-expansion is still much 
better. In Figure~\ref{fig::vs_jps2011} we give a summary of search performance, in terms
...
fast, optimal, online and require no extra memory vs. JPS 2011.
...
SUB~\cite{uras13} is a recent pathfinding technique from the literature.
As one of the joint winners of the 2012 Grid-based Path Planning Competition (GPPC) 
SUB has been shown to be very fast. 
We consider two variants: SUB-S (S for Simple) and SUB-TL
(TL for Two Level). The former is guaranteed optimal while the latter is not.
To evaluate SUB-S and SUB-TL we used the authors' original C++ implementation which 
we obtained from~\url{http://gppc-2012.googlecode.com/svn/trunk/entries/SUB-a/}.
...
The main advantage is performance: JPS+ and JPS+ (P) we can improve the search times of 
