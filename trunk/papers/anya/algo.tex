...
and $b$.
\end{defi}
Given a traversable point $p$, found along a particular row of 
the grid, we construct an interval as follows:
If $p$ is a discrete corner point or the start or target
point, we construct a closed interval $I = [a, b]$ where
$a = b = p$.
Otherwise we project two horizontal lines away from $p$:
$a \leftarrow p$ and $p \rightarrow b$. The corresponding
interval $I = (a, b]$ is half-open 
(resp. $I = (a, b)$ is open) if one of
its endpoints $a$ (resp. $a$ and $b$) is the first of:
(i) the start or target point or (ii) a discrete corner point;
found along the straight line from $p$.
Anya constructs intervals on-the-fly; this is in contrast
to other similar algorithms (e.g. Continuous Dijkstra) that must
preprocess the map before answering any queries. 
\\
\begin{defi}
...
The identity of the root node is either the start point or the most
