...
%optimality of $\pi$.
\end{proof}
Lemma~\ref{lemma::opt_is_taut} tells us that optimality does not depend on 
generating all successors during search. Only those successors
reachable by a local path which is taut need to be considered.
Anya generates exactly these successors.
\begin{lemm}
For any search node $(I, r)$ and any maximal interval $J$
found on the same grid row as $I$ or one immediately adjacent:
\textbf{if} $J$ contains a contiguous subset of traversable 
points $p$ s.t. $\forall p \in I' \subseteq J$ there is a taut path from $r$
\textbf{then} $(I', r')$ is a successor of $(I, r)$.
\end{lemm}
\begin{proof}
%In the case of the start node, every visible point
%and some others are generated.
%There are no no non-taut successors.
Sketch only. This needs fixing.
By Definition 3 the only successors not
generated are those reached by a non-taut path
or by a taut path that passes through more than one
interval.
If we pass through more than one interval we either
do so to reach a point which is not on the same row
or an adjacent row as $I$ or we pass through an interval
on the same row. If we pass through an interval on the same
row we do not need to generate the successor; it will
be reached by a taut path from one of our successors.
\end{proof}
Finally, we will show that if Anya returns a path the length
of that path is optimal.
%Finally, we will show that if Anya returns a path between any
%start-target pair, that path is optimal.
\begin{theorem}
Anya is optimal.
\end{theorem}
\begin{proof}
