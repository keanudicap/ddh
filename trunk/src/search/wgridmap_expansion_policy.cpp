		...
		warthog::problem_instance* problem)
{
    ...
    // transition costs to each neighbour are calculated as the average of 
    // terrain values for all tiles touched by the agent when moving.
    // CAVEAT EMPTOR: we only perform integer maths so there is some 
    // precision loss.
